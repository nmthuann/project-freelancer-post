...
import { IJobPostDetailService } from '../job-post-detail/job-post-detail.service.interface';
import { JobPostDto } from '../job-post/dtos/job-post.dto';
import { JobPostDetailDto } from '../job-post-detail/dtos/job-post-detail.dto';
import { CategoryDetailDto } from '../category-detail/dtos/category-detail.dto';
...
export class PostService {
  ...
  async createPost(email: string, postDto: PostDto): Promise<PostDto> {
    try {
      // check name category is exsit
      const getNameCategoryDetail: CategoryDetailDto =
        await this.categoryDetailService.getIdByCategoryDetailName(
          postDto.category_detail_name,
        );
      ...
      const createOneJPDetail = await this.jobPostDetailService.createOne(
        newJobPostDetail,
      );
  ...
  async getPostByEmail(email: string): Promise<PostDto[]> {
    const getPostByEmail = await this.postModel.find({
  ...
  async getPostsByCategoryDetailId(
    ...
    const jobPost = await this.jobPostService.getPostsByCategoryDetailId(
      category_detail_id,
    );
  ...
  async getPostForOrder(): Promise<PostOrderDto> {
