...
export class PostServiceProxy implements IPostService {
  ...
  async createPost(email: string, post: PostDto): Promise<PostDto> {
  ...
  async getPostsByCategoryDetailId(
    ...
    return await this.postService.getPostsByCategoryDetailId(
      category_detail_id,
    );
  }
  ...
  async getPostForOrder(data: any): Promise<PostOrderDto> {
    console.log('Proxy Service - getPostForOrder', data);
    return await this.postService.getPostForOrder();
  }
